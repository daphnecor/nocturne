"""Create RL environment from a single scene."""
...
class NocturneEnv(Env):
    def __init__(self, path_to_scene, scene_name, scenario_config, valid_veh_dict, cfg):
        """
        Make a Nocturne environment with discrete action space from a given traffic scenario.

        Args
        ----
        traffic_scene (str): path to the traffic scene.
        scenario_config (dict): simulation configurations.
        cfg (dict): other environment configurations.
        """
        super().__init__()
        self.path_to_scene = path_to_scene
        self.scene_name = scene_name
        self.scenario_config = scenario_config
        self.valid_veh_dict = valid_veh_dict
        self.cfg = cfg
        ...
        # Create mappping from joint action index to action values
        self.idx_to_actions = {}
        i = 0
        ...
        for veh_obj in self.controlled_vehicles:
            ...
            if done_dict[veh_id]:
                ...
                if (
                    ...
                    and self.cfg.get("remove_at_goal", True)
                ) or (
    ...
    def reset(self):
        ...
        while not enough_vehicles:
            self.simulation = Simulation(
                self.path_to_scene + self.scene_name,
            ...
            for veh_obj in self.simulation.getScenario().getObjectsThatMoved():
                ...
                if (
                    norm < self.cfg["rew_cfg"]["goal_tolerance"]
                ...
                ############################################"""
                if (
                    self.scene_name in self.valid_veh_dict
                    and veh_obj.getID() in self.valid_veh_dict[self.scene_name]
                ):
            ...
            for vehicle in temp_vehicles:
                ...
                # as controlled vehicles
                    ...
                    and curr_index < self.cfg["max_num_vehicles"]
                ):
        ...
        if self.cfg["subscriber"]["keep_inactive_agents"]:
            ...
            num_missing_agents = max(0, self.cfg["max_num_vehicles"] - len(obs_dict))
    ...
    def get_observation(self, veh_obj):
        ...
        if (
            self.cfg["subscriber"]["use_ego_state"]
            and self.cfg["subscriber"]["use_observations"]
        ):
            obs = np.concatenate(
                (
                    ego_obs,
            ...
            self.cfg["subscriber"]["use_ego_state"]
            and not self.cfg["subscriber"]["use_observations"]
        ):
            obs = ego_obs
        else:
